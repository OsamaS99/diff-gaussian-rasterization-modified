...
renderCUDA(
    ...
    int* __restrict__ splat_frequency // ADDED
)
{
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
    {
        ...
        for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
        {
            ...
            // This splat contributes to this pixel, so increment its frequency.
            // Use an atomic operation because multiple threads might update
            // the same splat simultaneously.
            atomicAdd(&splat_frequency[collected_id[j]], 1);

            // Update pixel color using the features.
